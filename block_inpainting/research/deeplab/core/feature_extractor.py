...
def _preprocess_zero_mean_unit_range_zero_one_hints(inputs):
    return _preprocess_zero_mean_unit_range(inputs, legacy=False)


def _preprocess_zero_mean_unit_range(inputs, legacy=True):
    """Map image values from [0, 255] to [-1, 1].

    New (legacy = False): map first three channels of input to [-1, 1] and map remaining channels to [0, 1]
    """

    input_shape = inputs.get_shape().as_list()
    assert len(input_shape) == 4

    print("feature_extractor.py: inputs shape is {}".format(input_shape))
    if input_shape[-1] == 3 or legacy:
        output = (2.0 / 255.0) * tf.to_float(inputs) - 1.0
    else:
        print("feature_extractor.py: inputs > 3 channels; assuming channels > 3 are hints")
        img = inputs[...,:3]
        print(img.get_shape().as_list())
        img = (2.0 / 255.0) * tf.to_float(img) - 1.0

        hints = inputs[...,3:]
        print(hints.get_shape().as_list())
        hints = (1.0 / 255.0) * tf.to_float(hints)
        output = tf.concat([img, hints], axis=-1)

    assert output.get_shape().as_list() == inputs.get_shape().as_list()

    return output


_PREPROCESS_FN = {
    'mobilenet_v2': _preprocess_zero_mean_unit_range_zero_one_hints,
    'resnet_v1_50': _preprocess_subtract_imagenet_mean,
    'resnet_v1_50_beta': _preprocess_zero_mean_unit_range_zero_one_hints,
    'resnet_v1_101': _preprocess_subtract_imagenet_mean,
    'resnet_v1_101_beta': _preprocess_zero_mean_unit_range_zero_one_hints,
    'xception_41': _preprocess_zero_mean_unit_range_zero_one_hints,
    'xception_65': _preprocess_zero_mean_unit_range_zero_one_hints,
    'xception_71': _preprocess_zero_mean_unit_range_zero_one_hints,
}
...
def extract_features(images,
    ...
    """
    if force_dropout:
        assert 'xception_65' in model_variant, 'Force dropout only works with xception_65 for now.'
