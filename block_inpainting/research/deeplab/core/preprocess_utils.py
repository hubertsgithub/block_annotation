...
def randomly_scale_image_and_label(image, label=None, hint=None, scale=1.0):
    ...
    Args:
      ...
      hint: hint with shape [height, width, c]. For boundary hints, c=2 (boundary/non-boundary hints)
    ...
    with tf.name_scope(scope, 'resize_to_range', [image]):
        ...
        if label is not None:
            if label_layout_is_chw:
                assert False, "I don't think this code makes sense."
        ...
        if hint is not None:
            if label_layout_is_chw:
                assert False, "I don't think this code makes sense."
                # Input hint has shape [channel, height, width] (hints assumed
                # to have same format as label)
                ...
                # Input hint has shape [height, width, channel].
