...
def _get_data(data_provider, dataset_split):
    ...
    label = None
    test_labels = True  # True if we want to get labels for test set.
    if test_labels or dataset_split != common.TEST_SET:
        if test_labels and dataset_split == common.TEST_SET:
            print("***Getting labels for test set.")
        label, = data_provider.get([common.LABELS_CLASS])


    class_hint = None
    if common.CLASS_HINT in data_provider.list_items():
        class_hint, = data_provider.get([common.CLASS_HINT])
...
def get(dataset,
    ...
    dataset split, (2) input_preprocess which preprocess the raw data, and (3) the
    ...
    output = _get_data(data_provider, dataset_split)
    image = output['image']
    label = output['label']
    class_hint = output['class_hint']
    ...
    if class_hint is not None:
        if class_hint.shape.ndims == 2:
            class_hint = tf.expand_dims(class_hint, 2)
        elif class_hint.shape.ndims == 3 and class_hint.shape.dims[2] == 1:
            pass
        else:
            raise ValueError('Input class_hint shape must be [height, width], or '
                             '[height, width, 1].')

    if class_hint is not None:
        print('*** Input generator: class hint... ***')
        hint = class_hint
        assert hint.shape.dims[2] == 1

    original_image, image, label,\
        hint = input_preprocess.preprocess_image_and_label(
            image=image,
            label=label,
            hint=hint,
