...
def predict_labels_multi_scale(images,
        ...
        for output in sorted(outputs_to_scales_to_logits):
            ...
            if add_flipped_images:
                ...
                logits_reversed = tf.image.resize_bilinear(
                    tf.reverse_v2(scales_to_logits_reversed[
                                  _MERGED_LOGITS_SCOPE], [2]),
                    tf.shape(images)[1:3],
                    align_corners=True)
                outputs_to_predictions[output].append(
    ...
    if also_return_logits:
        return outputs_to_predictions, outputs_to_scales_to_logits
    else:
...
def predict_labels(images, model_options, image_pyramid=None,
                   also_return_logits=False,
                   also_return_features=False,
                   force_dropout=False,
                   force_dropout_only_branch=False,
                   keep_prob=0.5):
    """Predicts segmentation labels.
    ...
    outputs_to_scales_to_logits = multi_scale_logits(
        ...
        force_dropout_only_branch=force_dropout_only_branch,
        keep_prob=keep_prob)
    ...
    if also_return_features:
        ## TODO: This is hacked up.
        print( "WARNING: also_return_features is hacked up. Do not rely on this.")
        assert not also_return_logits
    ...
    if also_return_logits:
        return predictions, outputs_to_scales_to_logits
    else:
        return predictions
    ...
    if not model_options.aspp_with_batch_norm:
        ...
        with slim.arg_scope(
            ...
            with slim.arg_scope([slim.batch_norm], **batch_norm_params):
                ...
                features = slim.dropout(inputs=features,
                                        is_training=force_dropout,
                                        keep_prob=keep_prob)
...
def _get_logits(images,
    ...
    """
    if force_dropout_only_branch and not force_dropout:
        raise Exception("If force_dropout_only_branch is True, you must set force_dropout True!")
    if force_dropout_only_branch:
        raise Exception("Branch does not include dropout at the moment! Look at code.")

    # Always force dropout on branch if force_dropout
    force_dropout_branch = force_dropout #or (force_dropout and force_dropout_only_branch)
    # Only force dropout on remainder of network if not force_dropout_only_branch
    force_dropout = (force_dropout and (not force_dropout_only_branch))
    ...
    for output in sorted(model_options.outputs_to_num_classes):
        outputs_to_logits[output] = _get_branch_logits(
            ...
            keep_prob=keep_prob,
            force_dropout=False)
            #force_dropout=force_dropout_branch)

    return outputs_to_logits

def _get_features_after_decoder(images,
                                model_options,
                                weight_decay=0.0001,
                                reuse=None,
                                is_training=False,
    ...
    assert model_options.decoder_output_stride is not None
    if model_options.decoder_output_stride is not None:
        decoder_height = scale_dimension(model_options.crop_size[0],
                                         1.0 / model_options.decoder_output_stride)
        decoder_width = scale_dimension(model_options.crop_size[1],
            ...
            with tf.variable_scope(_DECODER_SCOPE, _DECODER_SCOPE, [features]):
                ...
                if feature_list is None:
                    ...
                    for i, name in enumerate(feature_list):
                        ...
                        end_points[feature_name] = slim.dropout(inputs=end_points[feature_name],
                                                                keep_prob=keep_prob,
                                                                is_training=force_dropout)
                        ...
                        if decoder_use_separable_conv:
                            decoder_features = slim.dropout(inputs=tf.concat(decoder_features_list, 3),
                                                            keep_prob=keep_prob,
                                                            is_training=force_dropout)
                            ...
                            decoder_features = slim.dropout(inputs=decoder_features,
                                                            keep_prob=keep_prob,
                                                            is_training=force_dropout)
                            decoder_features = _split_separable_conv2d(
                                decoder_features,
