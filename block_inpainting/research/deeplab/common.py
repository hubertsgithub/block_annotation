...
flags.DEFINE_enum('model_variant', 'mobilenet_v2',
                  ['xception_41', 'xception_65', 'mobilenet_v2'], 'DeepLab model variant.')
...
flags.DEFINE_boolean('input_hints', False, 'Use input hints.')

flags.DEFINE_list('hint_types', ['class_hint'], 'Hint types to use. Options are: dynamic_block_hint. Others are deprecated.')

flags.DEFINE_integer('dynamic_block_hint_B', 10,
                     'Divide image into B*B grid.')

flags.DEFINE_float('dynamic_block_hint_p', 0.5,
                     'Sample p (0<p<=1) blocks from B*B grid')

flags.DEFINE_boolean('force_dropout', False,
                     'Add dropout to every layer.')
flags.DEFINE_boolean('force_dropout_only_branch', False,
                     'Add dropout to just branch logits (overrides force_dropout).')
flags.DEFINE_float('keep_prob', 0.8, 'dropout keep probability')
...
OUTPUT_TYPE_BOUNDARY_DISTANCE_MAP = 'boundary_distance_map'
OUTPUT_TYPE_BOUNDARY_MAP = 'boundary_map'
OUTPUT_TYPE_ALEATORIC_UNCERT = 'aleatoric_uncertainty'
